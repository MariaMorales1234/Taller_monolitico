...
if (!$materia || !$estudiante) {
    die("⚠️ No se proporcionaron los parámetros necesarios (materia y estudiante).");
}

$controller = new NotaController();
$nota = $controller->mostrar($materia, $estudiante);
...
<div class="container mt-4">
    ...
    <div class="mt-3">
        <a href="edit.php?materia=<?= urlencode($nota['materia']) ?>&estudiante=<?= urlencode($nota['estudiante']) ?>" class="btn btn-warning">Editar</a>
        <a href="index.php" class="btn btn-secondary">Volver a la lista</a>
        <a href="delete.php?materia=<?= urlencode($nota['materia']) ?>&estudiante=<?= urlencode($nota['estudiante']) ?>" class="btn btn-danger" onclick="return confirm('¿Estás seguro de eliminar esta nota? Esta acción no se puede deshacer.');">Eliminar</a>
    </div>
</div>
</body>
</html>
